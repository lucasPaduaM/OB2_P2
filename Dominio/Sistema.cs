...
namespace Social.NetWork.Dominio
{
    public class Sistema
    {
        ...
        public void PrecargaDeDatos()
        {
            ...
            //valor de aceptacion
            _posts[0].valorDeAceptacion = _posts[0].CalcularVA(_posts[0].likes, _posts[0].dislikes);
            _posts[1].valorDeAceptacion = _posts[1].CalcularVA(_posts[1].likes, _posts[1].dislikes);
            _posts[2].valorDeAceptacion = _posts[2].CalcularVA(_posts[2].likes, _posts[2].dislikes);
        }
        ...
        public void CrearReaccion(Reaccion nuevaReaccion, Post post)
        {
            if (post.YaReacciono(nuevaReaccion.miembroReaccion))
            {
                Reaccion ultimaReaccion = post.GetUltimaReaccion(nuevaReaccion.miembroReaccion);
                if (ultimaReaccion.TipoReaccion == nuevaReaccion.TipoReaccion)
                {
                    ...
                    int pos = post.GetPosReaccion(ultimaReaccion.miembroReaccion);
                    post.listaReacciones.RemoveAt(pos);
                }
                else if (ultimaReaccion.TipoReaccion != nuevaReaccion.TipoReaccion)
                {
                    if (nuevaReaccion.TipoReaccion.Equals(Reacciones.Like))
                    {
                        ...
                        int pos = post.GetPosReaccion(ultimaReaccion.miembroReaccion);
                        post.listaReacciones.RemoveAt(pos);
                        post.listaReacciones.Add(nuevaReaccion);
                    }
                    else if (nuevaReaccion.TipoReaccion.Equals(Reacciones.Dislike))
                    {
                        post.dislikes++;
        ...
        public List<Post>FiltrarPosts(string titulo, int VA)
        {
            List<Post> listTemp = new List<Post>();
            foreach(Post post in this.GetPosts())
            {
                if(post.titulo.Contains(titulo) && post.valorDeAceptacion > VA)
                {
                    listTemp.Add(post);
                }
            }
